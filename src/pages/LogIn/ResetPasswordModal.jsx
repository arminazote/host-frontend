...
const ResetPasswordModal = ({ isOpen, onClose, onOtpVerified }) => {
  ...
  const handleRequestOtp = async () => {
    ...
    try {
      const response = await axios.post(
        "https://api.ranibaji.com/api/v1/auth/send-otp",
        {
          numbers: `${phoneNumber}`,
        }
      );
        ...
        const interval = setInterval(() => {
          setTimeRemaining((prev) => {
            if (prev <= 1) {
              ...
              setCooldown(false); // End cooldown after 2 minutes
              return 0;
            }
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      const response = await axios.post(
        "https://api.ranibaji.com/api/v1/auth/verify-phone",
        {
          numbers: `${phoneNumber}`,
