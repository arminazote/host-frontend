...
const AddSecondaryPhoneModal = ({ isOpen, onClose }) => {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_API_BASE_URL}/auth/add-secondary`,
        {
      ...
      if (response.status === 200) {
        ShowToast({ success: "Secondary phone number added successfully" });
...
export default AddSecondaryPhoneModal;
